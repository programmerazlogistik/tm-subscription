...
export const MOCK_DATA = {
  ...
  Data: {
    ...
    usageOptions: [
      ...
      {
        value: "view_bid_participant",
        name: "Melihat Daftar Peserta Lelang",
      },
      {
        value:
          "view_transporter_contact_pricelist,view_transporter_contact_bid",
        name: "Melihat Contact Transporter",
      },
      {
        value:
          "view_transporter_profile_pricelist,view_transporter_profile_bid",
        name: "Melihat Profile Transporter",
      },
    ],
  },
...
export const getUsageHistory = async ({
  ...
  if (USE_MOCK) {
    ...
    result = await fetcherTM.get(
      `/v1/tm/buyer_subscription/usage-history?period=${period}&page=${page}&limit=${limit}${searchParam}${usageTypeParam}${startDateParam}${endDateParam}${roleParam}`
    );
  }
  return result.data;
};
...
export const useGetUsageHistory = ({
  ...
  return useSWR(
    accessToken
      ? `get-usage-history?period=${period}&page=${page}&limit=${limit}&search=${search}&usageType=${usageType}&startDate=${startDate}&endDate=${endDate}&role=${role || ""}`
      : null,
    () =>
      getUsageHistory({
        period,
